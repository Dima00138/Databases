...
---1 1.	Создайте процедуру, которая добавляет заказ. Обработайте возможные ошибки.
...
as
    begin
        begin try
            INSERT ORDERS(ORDER_NUM, ORDER_DATE, CUST, REP, MFR, PRODUCT, QTY, AMOUNT) values (110 ,getDate(), @cust, @rep, @mfr, @product, @qty, @amount);
            return 1;
        end try
        begin catch
            print 'Возникла ошибка -' + ERROR_MESSAGE();
            Raiserror('errorProc', 16,1);
            return;
        end catch
...
go
Create TRIGGER to_orders_tr
on ORDERS instead of INSERT
as
    if (select amount from inserted) < 0
    begin
         Raiserror('errortrigger', 16, 1);
         rollback transaction;
    end
go

Exec Add_order 2113, 101, 'REI', '2A44R', 5, -100.00;
...
CREATE FUNCTION funcNumOrdersByCustomer(@cust INTEGER, @dateFrom DATE,
                                        @dateTo DATE)
    RETURNS @o_table TABLE (numOrders INTEGER, monthNum INTEGER)
AS
BEGIN
    ...
    SET @c_table = CURSOR FOR
        SELECT COUNT(*) AS numOrders, MONTH(ORDER_DATE) AS monthNum
        FROM ORDERS
        WHERE ORDER_DATE BETWEEN @dateFrom AND @dateTo AND CUST = @cust
        GROUP BY MONTH(ORDER_DATE);

    OPEN @c_table;
    FETCH NEXT FROM @c_table INTO @numOrders, @monthNum;
    WHILE @@FETCH_STATUS = 0
        BEGIN
            INSERT INTO @o_table (numOrders, monthNum)
            VALUES (@numOrders, @monthNum);
            FETCH NEXT FROM @c_table INTO @numOrders, @monthNum;
        END;
    CLOSE @c_table;
    DEALLOCATE @c_table;
...
DECLARE @n_i INTEGER;
DECLARE @n_n INTEGER;
DECLARE @c_kek CURSOR;

SET @c_kek = CURSOR FOR
    SELECT numOrders, monthNum
    FROM dbo.funcNumOrdersByCustomer(2111, '2007-01-01', '2007-12-31');

OPEN @c_kek;
FETCH NEXT FROM @c_kek INTO @n_i, @n_n;
WHILE @@FETCH_STATUS = 0
    BEGIN
        PRINT 'In ' + CAST(@n_n AS VARCHAR(2)) + ' month was ' + CAST(@n_i AS VARCHAR(10)) + ' orders.';
        FETCH NEXT FROM @c_kek INTO @n_i, @n_n;
    END;
CLOSE @c_kek;
DEALLOCATE @c_kek;
...
CREATE or alter PROCEDURE procListNotOrdered(@dateYear INTEGER)
AS
BEGIN
    DECLARE @c_query CURSOR;
    DECLARE @product_id varchar(5);
    DECLARE @description VARCHAR(255);
    DECLARE @mfr_id varchar(3);

    SET @c_query = CURSOR FOR
        SELECT DISTINCT p.product_id, p.description, p.mfr_id
        FROM products p
                 LEFT JOIN orders o
                           ON p.product_id = o.product AND p.mfr_id = o.mfr AND YEAR(o.order_date) = @dateYear
        WHERE o.order_num IS NULL OR YEAR(o.order_date) <> @dateYear;

    OPEN @c_query;
    FETCH NEXT FROM @c_query INTO @product_id, @description, @mfr_id;
    WHILE @@FETCH_STATUS = 0
        BEGIN
            PRINT @description + ' ' + @product_id + ' (' + @mfr_id + ')';
            FETCH NEXT FROM @c_query INTO @product_id, @description, @mfr_id;
        END;
...
CREATE FUNCTION funcHowMuchOrders(@yearAsChar CHAR(4), @customer VARCHAR(255))
    RETURNS INTEGER
AS
BEGIN
    DECLARE @n_counter INTEGER = 0;
    DECLARE @t_pattern VARCHAR(255) = '%';
    DECLARE @n_pattern INTEGER = 0;

    SET @t_pattern = CONCAT(@t_pattern, @customer);
    SET @t_pattern = CONCAT(@t_pattern, '%');

--     BEGIN
--         BEGIN TRY
--             SET @n_pattern = CAST(@customer AS INTEGER);
--         END TRY
--         BEGIN CATCH
--             SET @n_pattern = 0;
--         END CATCH;
--     END
    ...
    SELECT @n_counter = COUNT(*)
    FROM ORDERS o
             INNER JOIN CUSTOMERS c
                        ON o.CUST = c.CUST_NUM
    WHERE (@yearAsChar = CAST(YEAR(o.ORDER_DATE) AS CHAR(4)))
      AND ((c.CUST_NUM = @n_pattern) OR (UPPER(c.COMPANY) LIKE UPPER(@t_pattern)));

    RETURN @n_counter;
END;

BEGIN
    PRINT '2008, 2111: ' + CAST(dbo.funcHowMuchOrders('2008', '2111') AS VARCHAR(10));
    PRINT '2007, Orion: ' + CAST(dbo.funcHowMuchOrders('2007', 'Orion') AS VARCHAR(10));
END;
...
CREATE OR ALTER PROCEDURE procOrderTableHard5
...
BEGIN
    ...
    DECLARE @t_query NVARCHAR(100)
    DECLARE @errorMsg NVARCHAR(100)

    IF (@columnName NOT IN ('c1', 'c2'))
        BEGIN
            SET @errorMsg = 'Incorrect column name'
    ...
    IF (@orderType NOT IN ('ASC', 'DESC'))
        BEGIN
            SET @errorMsg = 'Incorrect order type'
            RAISERROR(@errorMsg, 16, 1)
            RETURN;
        END

    SET @t_query = 'DECLARE c_mem CURSOR dynamic FOR SELECT * FROM hard_5 ORDER BY ' + QUOTENAME(@columnName) + ' ' + @orderType;
    ...
    BEGIN TRY
        ...
        EXEC sp_executesql @t_query;

        OPEN c_mem;

        FETCH NEXT FROM c_mem INTO @c1, @c2;

        DELETE FROM hard_5;

        print 'c1 ' + char(@c1) + ', c2 ' + char(@c2);

        WHILE @@FETCH_STATUS = 0
            BEGIN
                INSERT INTO hard_5 (c1, c2) VALUES (@c1, @c2);
                FETCH NEXT FROM c_mem INTO @c1, @c2;
            END
    ...
    END TRY
    BEGIN CATCH
        SET @errorMsg = 'POTH5 error: ' + ERROR_MESSAGE();
        ROLLBACK;
        RAISERROR(@errorMsg, 16, 1);
        RETURN;
    END CATCH;
...
Create TABLE hard_5 (c1 int, c2 int);
select * from hard_5;
Declare @i int = 0
begin
    while @i < 10
        begin
        Insert hard_5(c1,c2) values(@i, 10 - @i);
        Set @i = @i + 1;
        end;
end;

exec procOrderTableHard5 'c2', 'ASC';